...
def main():
    ...
    def win_adjusted(c):
        n = deck_games[c]
        if n == 0:
            return rating.get(c, 0.0)
        wr = deck_wins[c] / n
        return rating.get(c, 0.0) + args.win_weight * (wr - 0.5) * n / (n + 300.0)
    ...
    if args.table:
        ...
        values_win = ",".join(str(int(round(win_adjusted(c) * 100))) for c in sorted(names))
        # поправка из self-play (tools/arena/selfplay_cards.py) хранится в том же файле — сохраняем текущее значение
        packed_self = ""
        m = re.search(r'PackedSelf = "([^"]*)"', Path(args.table).read_text(encoding="utf-8")) if Path(args.table).exists() else None
        if m:
            packed_self = m.group(1)
        lines = [
            ...
            f"        private const string PackedSelf = \"{packed_self}\";",
            "",
